...
function qa_eval_from_file($eval, $filename)
{
	...
	if (strlen($php_errormsg)) {
		...
		@error_log('PHP AI more info: ' . $php_errormsg . " in eval()'d code from " . qa_html($filename));
	}
...
function qa_fatal_error($message)
{
	...
	echo 'AI fatal error:<p style="color: red">' . qa_html($message, true) . '</p>';
	@error_log('PHP AI fatal error: ' . $message);
